...
class HorseGrimaceDataset(Dataset):
    ...
    label_column : str
        Name of the CSV column containing the three-class pain score.
    ...
    def _validate_labels(self) -> None:
        ...
        if self.data[self.label_column].isna().any():
            raise ValueError(
                f"Missing values were found in "
                f"'{self.label_column}'."
            )
        ...
        try:
            ...
            raise ValueError(
                f"Labels in '{self.label_column}' "
                "must be integers."
            ) from error

        found_labels = set(
            self.data[self.label_column].unique()
        )

        invalid_labels = found_labels.difference(
            self.VALID_LABELS
        )
    ...
    def _resolve_image_path(
        ...
        """
        Convert a stored image path into a usable path.

        Absolute paths are used directly. Relative paths are joined
        with image_root when image_root is provided.
        """
    ...
    def __getitem__(
        ...
        if not image_path.exists():
            raise FileNotFoundError(
                f"Image not found for dataset index "
                f"{index}: {image_path.resolve()}"
            )
        ...
        return {
            ...
            "horse_id": str(
                row[self.horse_id_column]
            ),
            "face_region": str(
                row[self.face_region_column]
            ),
